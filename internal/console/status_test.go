...
func TestRootCN(t *testing.T) {
	id := &cryptosv1.Identity{ChainPem: leafPEM(t, "Example Root CA G1")}
	if got := console.RootCN(id); got != "Example Root CA G1" {
...
func TestIssuerCN(t *testing.T) {
	chain := leafPEM(t, "Example Issuing CA G1") + leafPEM(t, "Example Root CA G1")
	id := &cryptosv1.Identity{ChainPem: chain}
	if got := console.IssuerCN(id); got != "Example Root CA G1" {
		t.Fatalf("IssuerCN(2-cert chain) = %q, want the second subject CN", got)
	}
	self := &cryptosv1.Identity{ChainPem: leafPEM(t, "Example Root CA G1")}
...
func TestViewFromAPI(t *testing.T) {
	...
	if v.RootCN != "Example Root CA G1" || v.Role != "ROOT" || v.NodeStatus != "ESTABLISHED" || v.TPM != "SEALED" {
