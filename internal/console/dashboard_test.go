...
func TestRenderDashboardServing(t *testing.T) {
	v := console.View{
		RootCN: "Example Root CA G1", Role: "ROOT", NodeStatus: "ESTABLISHED",
		TPM: "SEALED", Uptime: 74 * time.Hour, Version: "phase-1-dev", Fleet: console.FleetConnected,
	}
	out := console.RenderDashboard(v, 64, 24)
	plain := stripSGR(out)
	for _, want := range []string{"CryptOS PKI", "ROOT", "Example Root CA G1", "ESTABLISHED", "SEALED", "connected", "^R"} {
...
func TestRenderDashboardIssuer(t *testing.T) {
	v := console.View{
		RootCN: "Example Issuing CA G1", Role: "INTERMEDIATE", Issuer: "Example Root CA G1",
	...
	for _, want := range []string{"Intermediate CA", "Issuer", "Example Root CA G1"} {
		if !strings.Contains(plain, want) {
			t.Fatalf("serving dashboard missing %q:\n%s", want, plain)
		}
	}
...
func TestRenderDashboardMaintenanceAndDegraded(t *testing.T) {
	...
	d := stripSGR(console.RenderDashboard(console.View{Degraded: true, RootCN: "Example Root CA G1", Role: "ROOT"}, 64, 24))
